...
"""
===========================================
Incorporating Synoptic Data's API Services
===========================================
...
of Utah's MesoWest program, and now serving as one of the National Mesonet Program's
lead subcontractors, Synoptic provides access to real-time and historical data from
>100,000 stations across North America with low latency through it's
...
integrating with Synoptic's API data services. Tailor your query your specifications,
and the package will send a data request to Synoptic's API, parse the returned data, and
treat units in a pint-compatible format to facilitate incoporating Synoptic's deep database
...
# package, we broadly group parameters in to dictionaries relating to 1) station/variable,
...
#########################################################################
# The units object is a dictionary specifying the units of each variable:
ut_units
...
#########################################################################
# Key values in the units dictionary match the dataframe columns to ease attaching
# units to the data:
...
#########################################################################
# Timing requests must be in UTC, but data can be returned in the local time zone. Further
# we can also specify units in the API request. Set these optional parameters:
...
#########################################################################
# A quick glimpse of the return shows the expected... warm temps:
...
#########################################################################
# The returned data object containes timeseries data for each site:
stn_data
...
# identify the interval (*from*, *to*) over which the results are measured/computed. Here's an
